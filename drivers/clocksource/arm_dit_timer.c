 ...
 *
 *  Copyright (C) 2011-2014  RC Module
 *
...
static int dit_next_event(unsigned long cycles, struct clock_event_device *evt_dev)
{
...
static void dit_set_mode(enum clock_event_mode mode, struct clock_event_device *evt_dev)
{
	struct dit_timer *timer = container_of(evt_dev,
		struct dit_timer, evt);
	unsigned long ctrl;
...
static void __init dit_time_init(struct device_node *node)
{
	...
	printk("DIT: Timer system initialized\n");
