...
static int rcm_pll_wait_for_stable(struct rcm_pll *pll)
{
    unsigned int val = 0;
    int ret;
    ret = regmap_read_poll_timeout(pll->regmap, RCM_PLL_CTRL, val, (val&1) == 0, 0, RCM_PLL_STABILITY_TIMEOUT);
    if(!ret)
        ret = regmap_read_poll_timeout(pll->regmap, RCM_PLL_STATE, val, val == 1, 0, RCM_PLL_STABILITY_TIMEOUT);
    TRACE("%d", ret);
    return ret;
}
...
static int rcm_pll_enable(struct clk_hw *hw)
{
    ...
    if(val == 0)
    {
        regmap_update_bits(pll->regmap, RCM_PLL_CTRL, BIT(1), BIT(1)); // enable PLL                    
...
static void rcm_pll_disable(struct clk_hw *hw)
{
    ...
    if(val == 0)
    {
        regmap_update_bits(pll->regmap, RCM_PLL_CTRL, BIT(1), 0); // disable PLL                    
        regmap_write(pll->regmap, RCM_PLL_WR_LOCK, 0); // lock back
    }
    spin_unlock_irqrestore(&pll->lock, flags);  
}
...
static unsigned long rcm_get_rate_by_divisors(struct device* dev, unsigned long parent, unsigned int prdiv, unsigned int fbdiv, unsigned int psdiv)
{
    ...
    {
        default:
        case 0: postdiv = 1; break;
        case 1: postdiv = 2; break;
        case 2: postdiv = 4; break;
        case 3: postdiv = 8; break;
    }
			...
			unsigned long parent_rate)
{
    ...
    rate = rcm_get_rate_by_divisors(pll->dev, parent_rate, prdiv, fbdiv, psdiv);
			...
			unsigned long *prate)
{
    ...
    for(i = 0; i < pll->freq_num; i++)
    {
        ...
        new_rate = rcm_get_rate_by_divisors(pll->dev, *prate, prdiv, fbdiv, psdiv);
        if(!new_rate)
            return 0;

        if(abs((long)new_rate - rate) < abs((long)new_rate - nearest_freq))
			...
			unsigned long parent_rate)
{
    ...
    {
        unsigned int psdiv, fbdiv, prdiv, new_rate;
        new_rate = rcm_get_rate_by_divisors(pll->dev, parent_rate, pll->freqs[i].prdiv, pll->freqs[i].fbdiv, pll->freqs[i].psdiv);
        if(rate == new_rate)
        {
            ...
            {
                unsigned int val = 1;
                TRACE("set new values %d, %d, %d", pll->freqs[i].prdiv, pll->freqs[i].fbdiv,pll->freqs[i].psdiv);
                spin_lock_irqsave(&pll->lock, flags);
                ...
                if(val == 0)
                {
                    regmap_read(pll->regmap, RCM_PLL_CTRL, &val);
                    TRACE("current CTRL: %08x", val);
                    regmap_read(pll->regmap, RCM_PLL_RST_CTRL, &val);
                    ...
                    regmap_update_bits(pll->regmap, RCM_PLL_CTRL, 1, 1); // restart PLL                    
                    regmap_write(pll->regmap, RCM_PLL_WR_LOCK, 0); // lock back
                    ...
                    dev_err(pll->dev, "unable to unlock PLL registers");
...
static int rcm_pll_probe(struct platform_device *pdev)
{
	...
	d->regmap =
		devm_regmap_init_mmio(dev, base, &rcm_pll_regmap_config);
	if (IS_ERR(d->regmap))
