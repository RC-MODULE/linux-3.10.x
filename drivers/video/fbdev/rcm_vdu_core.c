...
{
	/*const*/ phys_addr_t fb_base = NULL; //k1879_dt_get_fb_base();
...
//	vdu->fb_buffer_size = fb_size;
	...
	dev_info(&pdev->dev,"got fb address! %x\n", res.start);
	...
	dev_info(&pdev->dev,"fb_buffer_cpu %p, fb_buffer_dma %p, fb_buffer_size %x\n", vdu->fb_buffer_cpu, vdu->fb_buffer_dma, vdu->fb_buffer_size);
