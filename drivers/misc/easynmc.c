...
static long easynmc_ioctl(struct file *filp, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	case IOCTL_NMC3_ION2NMC:
	{
		...
		hndl = ion_import_dma_buf_fd(core->iclient, (int) fd);
		if (IS_ERR(hndl))
			return -EIO;

		//ret = ion_hys(core->iclient, hndl, &nmaddr, &len);
		struct ion_buffer *buffer = hndl->buffer;
		struct sg_table *table = buffer->priv_virt;
                struct page *page = sg_page(page);
                nmaddr = page_to_phys(page);
                len = buffer->size;
