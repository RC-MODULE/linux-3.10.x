...
#define DRIVER_NAME	"rcm_vdu_video"
#define CARD_NAME	"rcm_vdu"	/* FIXME: this should come from outsize */
...
static int vdu_video_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	start = mvd->core_device->video_buffer_pa;
...
static int __init video_dev_init(struct mvdu_device *core_device)
{
	...
	for (i = 0; i < mvd->num_bufs; i++) {
		...
		mbs->base_dma = core_device->video_buffer_dma + b->m.offset;
	}
...
MODULE_DESCRIPTION("RCM VDU V4L2 driver");
MODULE_LICENSE("GPL");
