...
#define TRACE(format, ... ) do { \
		if(0) { \
			printk("TRACE: " DRIVER_NAME "/%s:%d: " format "\n", \
				       ...
				       uint32_t offset, uint32_t data)
{
	...
	writel(data, chip->dmac.io + offset);
}
...
{
	return readl(chip->dmac.io + 0x4 +
			(channel * 2 + page) * sizeof(uint32_t)) & ~0x40000000;
}
...
{
	return readl(chip->dmac.io + 0x80);
}
...
static inline uint32_t dmac_read_dmaaddr(struct maudio_chip *chip)
{
	return readl(chip->dmac.io + 0x9C);
}
...
static void dmac_start(struct maudio_chip *chip, int channels)
{
	...
	for (i = 0; i < channels; ++i) {
		...
		TRACE("ch%d: dma 0x%08X-0x%08X 0x%08X-0x%08X", i,
			(unsigned int) page0_start, (unsigned int) page0_end, (unsigned int) page1_start, (unsigned int) page1_end);
...
static int i2s_pcm_open(struct snd_pcm_substream *stream)
{
	static struct snd_pcm_hardware hw = {
		...
		.formats = SNDRV_PCM_FMTBIT_S16_LE /*| SNDRV_PCM_FMTBIT_S20_LE | SNDRV_PCM_FMTBIT_S24_LE*/ | SNDRV_PCM_FMTBIT_S32_LE
...
#define WAIT_WORDS 10

static void i2s_wait_for_data(struct maudio_chip *chip, int channels)
{
    int counter = WAIT_ITERATIONS;
    
    while (counter--)
    {        
        u32 stat0, stat1, ctw0, ctw1, ctw2, ctw3;

        schedule();
        stat0 = i2s_read_stat0(chip);
        stat1 = i2s_read_stat1(chip);
        ctw0 = stat0 & 0xff;
        ctw1 = (stat0>>16) & 0xff;
        ctw2 = stat1 & 0xff;
        ctw3 = (stat1>>16) & 0xff;

        printk("TRACE: wait %02X, %02X, %02X, %02X", ctw0, ctw1, ctw2, ctw3);

        if(ctw0 < WAIT_WORDS)
            continue;

        if(channels > 2 && ctw1 < WAIT_WORDS)
            continue;

        if(channels > 4 && ctw2 < WAIT_WORDS)
            continue;

        if(channels > 6 && ctw2 < WAIT_WORDS)
            continue;

        printk("TRACE: finish wait");
        break;
    }
}
...
static int i2s_pcm_trigger(struct snd_pcm_substream *stream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
        ...
        schedule();
        //i2s_wait_for_data(chip, stream->runtime->channels);
...
{
	TRACE("0x%08X", data);
	writel(data, chip->spdif.io + 0x18);
}
...
static inline void spdif_wait_full_fifo(struct maudio_chip *chip)
{
	...
	TRACE("0x84 0x%08X", readl(chip->dmac.io + 0x84));
	TRACE("0x88 0x%08X", readl(chip->dmac.io + 0x88));
	TRACE("0x8C 0x%08X", readl(chip->dmac.io + 0x8C));
	TRACE("0x90 0x%08X", readl(chip->dmac.io + 0x90));
	TRACE("0x94 0x%08X", readl(chip->dmac.io + 0x94));
	TRACE("0x98 0x%08X", readl(chip->dmac.io + 0x98));
	TRACE("0x9C 0x%08X", readl(chip->dmac.io + 0x9C));
	TRACE("0xA0 0x%08X", readl(chip->dmac.io + 0xA0));
	TRACE("0xA4 0x%08X", readl(chip->dmac.io + 0xA4));
	TRACE("0xA8 0x%08X", readl(chip->dmac.io + 0xA8));
	TRACE("0xAC 0x%08X", readl(chip->dmac.io + 0xAC));
	TRACE("0xB0 0x%08X", readl(chip->dmac.io + 0xB0));
	TRACE("0xB4 0x%08X", readl(chip->dmac.io + 0xB4));
	TRACE("0xB8 0x%08X", readl(chip->dmac.io + 0xB8));
	TRACE("0xBC 0x%08X", readl(chip->dmac.io + 0xBC));
	TRACE("0xC0 0x%08X", readl(chip->dmac.io + 0xC0));
	TRACE("0xC4 0x%08X", readl(chip->dmac.io + 0xC4));
	TRACE("0x%08X", spdif_read_stat(chip));
}
...
static void release_mem_resource(struct mem_resource *ret)
{
	...
	TRACE("releasing mem region(0x%08X,0x%08X)",
	      (unsigned int) ret->res.start, (unsigned int) resource_size(&ret->res));
}
			  ...
			  struct snd_pcm_ops *ops)
{
    ...
    printk("TRACE: maudio_pcm_new %s", name);
	...
	TRACE("area %p addr 0x%08X size %u", buf->area,
	      (unsigned int) buf->addr, buf->bytes);
...
static int maudio_driver_probe(struct platform_device *pdev)
{
    ...
    printk("TRACE: maudio_driver_probe");

	pdev->dev.coherent_dma_mask = DMA_BIT_MASK(32);

	if ((err = snd_card_new(&pdev->dev, -1, "maudio", THIS_MODULE, 
	     sizeof(struct maudio_chip), &card))) {
